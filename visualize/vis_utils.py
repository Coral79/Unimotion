...
class npy2obj:
    def __init__(self, npy_path, sample_idx, rep_idx, device=0, cuda=True):
        ...
        self.bs, self.njoints, self.nfeats, self.nframes = self.motions['motion'].shape
        # self.real_num_frames = self.motions['lengths'][self.absl_idx] ####chuqiao change back####
        ...
        self.local_text = self.motions['local_frame_txt'][self.absl_idx]
    ...
    def save_local_text(self, save_path ):
        save_txt = self.local_text[:self.real_num_frames]

        with open(save_path, 'w') as file:
            for line in save_txt:
                file.write(line + '\n')

    def save_global_text(self, save_path ):
        save_txt = self.motions['text'][0]
        print(save_txt)
