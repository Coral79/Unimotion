...
def get_joints_mask(join_names, local_txt_dim):
...
def get_batch_joint_mask(shape, joint_names, local_txt_dim):
    return expand_mask(get_joints_mask(joint_names, local_txt_dim), shape)

def get_local_text_mask(shape, local_txt_dim):
    num_feat = shape[1]
    njoints = num_feat - local_txt_dim
    TXT_MASK = np.concatenate(([False]*(njoints),[True] * local_txt_dim))  # True means use gt motion
    return expand_mask(TXT_MASK, shape)

def get_motion_mask(shape, local_txt_dim):
    num_feat = shape[1]
    njoints = num_feat - local_txt_dim
    TXT_MASK = np.concatenate(([True]*(njoints),[False] * local_txt_dim))  # True means use gt motion
    return expand_mask(TXT_MASK, shape)
...
def get_inpainting_mask(mask_name, shape, **kwargs):
    ...
    mask = np.zeros(shape)
    if 'gt_local_txt' in mask_names:
        mask = np.maximum(mask, get_local_text_mask(shape, **kwargs))

    if 'gt_motion' in mask_names:
        mask = np.maximum(mask, get_motion_mask(shape, **kwargs))
    ...
    return np.maximum(mask, get_batch_joint_mask(shape, mask_names, **kwargs))